...
function wpmc_screen() {
	...
	<div class='wrap'>
		...
		<h2>Media File Cleaner <?php by_jordy_meow(); ?></h2>
...
function wpmc_validate_pro( $subscr_id ) {
	...
	if ( !$post['success'] ) {
		if ( $post['message_code'] == "NO_SUBSCRIPTION" ) {
			$status = __( "Your serial does not seem right.", 'media-file-cleaner' );
		}
		else if ( $post['message_code'] == "NOT_ACTIVE" ) {
			$status = __( "Your subscription is not active.", 'media-file-cleaner' );
		}
		else if ( $post['message_code'] == "TOO_MANY_URLS" ) {
			$status = __( "Too many URLs are linked to your subscription.", 'media-file-cleaner' );
		}
		else {
			$status = __( "There is a problem with your subscription.", 'media-file-cleaner' );
		}
		update_option( 'wpmc_pro_serial', "" );
	...
	update_option( 'wpmc_pro_status', __( "Your subscription is enabled.", 'media-file-cleaner' ) );
	return true;
}
...
function wpmc_wp_enqueue_scripts () {
	...
	wp_enqueue_script( 'media-file-cleaner', plugins_url( '/media-file-cleaner.js', __FILE__ ), array( 'jquery' ), "3.0.0", true );
}
